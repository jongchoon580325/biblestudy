...
export default function PreviewPage() {
  ...
  const book = params?.book ? decodeURIComponent(params.book as string) : undefined;
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem(`bibleMaterials_${book}`);
    if (stored) {
      const arr: Material[] = JSON.parse(stored);
      const found = arr.find((m) => String(m.id) === String(materialId));
      if (found) {
        setMaterial(found);
        // 확장자 분기: 텍스트/마크다운/HTML/CSV는 base64 -> text 변환
        const ext = found.fileName.split('.').pop()?.toLowerCase() || '';
        if (["md","markdown","txt","html","csv"].includes(ext)) {
          if (found.content && found.content.startsWith("data:")) {
            // base64 -> text 변환
            fetch(found.content)
              .then(res => res.text())
